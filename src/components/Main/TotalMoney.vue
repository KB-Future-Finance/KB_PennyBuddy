          ...
          <div class="class">
              ...
              <div class="class-input">
                  <label for="income" class="radio">
                      <input type="radio" name="category_type" id="income" value="1" v-model="categoryType" @change="fetchCategories"/>
                      <span class="label"> 수입</span>
                  </label>
                  <label for="outcome" class="radio">
                      <input type="radio" name="category_type" id="outcome" value="2" v-model="categoryType" @change="fetchCategories"/>
          ...
          <div class="category">
              ...
              <div class="category-input">
                  ...
                  <ul class="list-items" v-show="isDropdownOpen">
                      <li class="item" v-for="category in filteredCategories" :key="category.category_id" @click.stop="selectItem(category)" :class="{ checked: selectedItem === category.category_name }">
                          <input type="radio" :value="category.category_name" v-model="selectedItem" class="radio"/>
...
const selectItem = (item) => {
  selectedItem.value = item.category_name;
  isDropdownOpen.value = false;
};

const getButtonText = computed(() => {
  return selectedItem.value ? selectedItem.value : '카테고리를 선택하세요';
});
...
const fetchCategories = async () => {
  if (categoryType.value) {
      ...
      categories.value = allCategories.filter(category => category.category_type == categoryType.value);
  } else {
      categories.value = [];
  }
};
